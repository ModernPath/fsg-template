...
// YTJ API v3 endpoint - the new stable endpoint for business lookups
 ...
 * Searches for companies by name or business ID
 */
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.log(`[CompanySearchAPI] Request received: query="${query}", limit=${limit}`);

    if (!query || query.length < 3) {
      console.warn(`[CompanySearchAPI] Invalid query: "${query}" (length: ${query?.length || 0})`);
    ...
    if (isBusinessId) {
      ...
      console.log(`[CompanySearchAPI] Using business ID search: ${endpoint}`);
    } else {
      // For name search, use the name parameter
      endpoint = `${YTJ_API_URL}?name=${encodeURIComponent(query)}&maxResults=${limit}`;
      console.log(`[CompanySearchAPI] Using name search: ${endpoint}`);
    }
    
    console.log(`[CompanySearchAPI] Calling YTJ API...`);
    const response = await fetch(endpoint, {
      headers: {
        'Accept': 'application/json',
        'User-Agent': 'Trusty-Finance/1.0'
      },
      // Extended timeout for better data collection
      signal: AbortSignal.timeout(30000) // 30 second timeout (increased from 10)
    });
    
    const responseTime = Date.now() - startTime;
    console.log(`[CompanySearchAPI] YTJ API responded in ${responseTime}ms with status ${response.status}`);
    
    if (!response.ok) {
      const errorText = await response.text();
      console.error(`[CompanySearchAPI] YTJ API error ${response.status}:`, errorText);
    ...
    console.log(`[CompanySearchAPI] Raw response length: ${responseText.length} characters`);
    ...
    try {
      ...
      console.error('[CompanySearchAPI] JSON parsing error:', parseError);
      console.error('[CompanySearchAPI] Invalid response:', responseText.substring(0, 500));
      return NextResponse.json({ 
        success: false, 
    ...
    console.log(`[CompanySearchAPI] YTJ API Response Structure: ${typeof data}, has companies: ${Boolean(data.companies)}, totalResults: ${data.totalResults || 'N/A'}`);
    
    let formattedResults: FormattedCompany[] = [];
    
    if (data && data.companies && Array.isArray(data.companies)) {
      console.log(`[CompanySearchAPI] Processing ${data.companies.length} companies from YTJ response`);
      ...
      formattedResults = data.companies.map((company: any, index: number) => {
        try {
          ...
          console.log(`[CompanySearchAPI] Processed company ${index + 1}: ${result.name} (${result.businessId})`);
          return result;
        } catch (companyError) {
          console.error(`[CompanySearchAPI] Error processing company ${index + 1}:`, companyError);
      ...
      console.log(`[CompanySearchAPI] Successfully processed ${formattedResults.length} companies`);
    } else {
      console.warn(`[CompanySearchAPI] No companies found in response or invalid structure`);
    }

    const totalTime = Date.now() - startTime;
    console.log(`[CompanySearchAPI] Request completed in ${totalTime}ms, returning ${formattedResults.length} results`);
    ...
    const totalTime = Date.now() - startTime;
    console.error(`[CompanySearchAPI] Unhandled error after ${totalTime}ms:`, error);
 ...
 * This allows us to access different address types separately
 */
