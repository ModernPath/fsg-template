...
export default async function BankLoanPage({ params }: Props) {
  const { locale } = params;
  await setupServerLocale(locale);
  const t = await getTranslations('Onboarding');
  const pageTitle = t('recommendationType.bank_loan', { default: 'Bank Loan' });
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      ...
      {/* Add specific content for Bank Loan here */}
    </main>
  );
} 