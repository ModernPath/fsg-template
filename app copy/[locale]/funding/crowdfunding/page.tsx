...
export default async function CrowdfundingPage({ params }: Props) {
  const { locale } = params;
  await setupServerLocale(locale);
  const t = await getTranslations('Onboarding');
  const pageTitle = t('recommendationType.crowdfunding', { default: 'Crowdfunding' });
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      ...
      {/* Add specific content for Crowdfunding here */}
    </main>
  );
} 